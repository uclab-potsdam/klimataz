...
const List = ({ lk, data, section, sectionFullName, handleClickOnList }) => {
  const agsCurrentCard = data.filter((d) => {
    return +d.AGS === lk.value;
  });
  ...
  const similarAgs = data.filter((d) => {
    return lk.value !== 0
      ? d[thirdKey] === agsCurrentCard[0][thirdKey] &&
  ...
  const agsRanking = agsCurrentCard[0][thirdKey].substring(
    0,
    agsCurrentCard[0][thirdKey].indexOf(' ')
  );
