                                    ...
                                    classesData.map((klass, k) => {
                                        return (
                                            <g key={k}>
                                                ...
                                                <text x={dimensions.width - marginWidth - 5} y={dimensions.height - klass.y2 + 15} textAnchor="end" fill="#484848">{klass.label}</text>
                                            </g>
                                        )
                                    })
                                }
                                ...
                                barChartData.map((bar, b) => {
                                    return (
                                        <g key={b} transform={`translate(${bar.year}, 0)`}>
                                            <rect x="0.5" y={dimensions.height - bar.kwh} height={bar.kwh - marginHeight} width="19" fill="none" stroke={bar.fill} fill-opacity="20%" />
                                            <rect x="0" y={dimensions.height - bar.kwh - 5} width="20" height="10" fill={bar.fill} rx="5" />
                            ...
                            barChartData.map((label, l) => {
                                return (
                                    <g key={l} transform={`translate(${marginWidth}, 0)`}>
                                        <text
                                            x={label.year + 20}
                                            y={dimensions.height - marginHeight + 10}
                                            textAnchor="middle"
                                        >
                                            {label.yearLabel}
                                        </text>
