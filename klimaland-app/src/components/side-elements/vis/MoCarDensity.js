  ...
  return (
    <div className={`car-density vertical-layout ${isThumbnail ? 'is-thumbnail' : ''}`}>
      <div className="description">
        <div className="title">
          <h3>
            <span>{lkData}</span>: Auf 100 Einwohner besitzen{' '}
            <span className="first-value">{totalCars}</span> ein Auto.{' '}
            <span className="second-value">{hybridCars}</span> davon sind Hybrid oder Elektro.
          </h3>
        </div>
            ...
            <g className="grid">
              {carSquares.map(function (sq, s) {
                return (
                  ...
                  >
                    <rect key={s} width={rectWidth} height={rectWidth} x="0" y="0" />
                  </g>
                );
              })}
            </g>
